...
import com.mlizhi.base.imageloader.core.assist.FailReason;
import com.mlizhi.base.imageloader.core.assist.FailReason.FailType;
...
import com.mlizhi.base.imageloader.core.decode.ImageDecodingInfo;
...
import com.mlizhi.base.imageloader.utils.IoUtils.CopyListener;
import java.io.File;
...
final class LoadAndDisplayImageTask implements Runnable, CopyListener {
    ...
    /* renamed from: com.mlizhi.base.imageloader.core.LoadAndDisplayImageTask.1 */
    ...
    /* renamed from: com.mlizhi.base.imageloader.core.LoadAndDisplayImageTask.2 */
    class C01242 implements Runnable {
        ...
        C01242(FailType failType, Throwable th) {
    ...
    /* renamed from: com.mlizhi.base.imageloader.core.LoadAndDisplayImageTask.3 */
    ...
    public void run() {
        if (!waitIfPaused() && !delayIfNeed()) {
            ...
            C0126L.m33d(LOG_START_DISPLAY_IMAGE_TASK, this.memoryCacheKey);
            if (loadFromUriLock.isLocked()) {
                C0126L.m33d(LOG_WAITING_FOR_IMAGE_LOADED, this.memoryCacheKey);
            }
            ...
            try {
                ...
                if (bmp == null || bmp.isRecycled()) {
                    ...
                    if (bmp != null) {
                        ...
                        if (this.options.shouldPreProcess()) {
                            C0126L.m33d(LOG_PREPROCESS_IMAGE, this.memoryCacheKey);
                            bmp = this.options.getPreProcessor().process(bmp);
                            if (bmp == null) {
                                C0126L.m34e(ERROR_PRE_PROCESSOR_NULL, this.memoryCacheKey);
                            }
                        }
                        if (bmp != null && this.options.isCacheInMemory()) {
                            C0126L.m33d(LOG_CACHE_IMAGE_IN_MEMORY, this.memoryCacheKey);
                ...
                C0126L.m33d(LOG_GET_IMAGE_FROM_MEMORY_CACHE_AFTER_WAITING, this.memoryCacheKey);
                if (bmp != null && this.options.shouldPostProcess()) {
                    C0126L.m33d(LOG_POSTPROCESS_IMAGE, this.memoryCacheKey);
                    bmp = this.options.getPostProcessor().process(bmp);
                    if (bmp == null) {
                        C0126L.m34e(ERROR_POST_PROCESSOR_NULL, this.memoryCacheKey);
                    }
                }
                checkTaskNotActual();
                checkTaskInterrupted();
    ...
    private boolean waitIfPaused() {
        ...
        if (pause.get()) {
            synchronized (this.engine.getPauseLock()) {
                if (pause.get()) {
                    C0126L.m33d(LOG_WAITING_FOR_RESUME, this.memoryCacheKey);
                    try {
                        this.engine.getPauseLock().wait();
                        C0126L.m33d(LOG_RESUME_AFTER_PAUSE, this.memoryCacheKey);
                    } catch (InterruptedException e) {
                        C0126L.m34e(LOG_TASK_INTERRUPTED, this.memoryCacheKey);
                        return true;
                    }
                }
            }
    ...
    private boolean delayIfNeed() {
        ...
        C0126L.m33d(LOG_DELAY_BEFORE_LOADING, Integer.valueOf(this.options.getDelayBeforeLoading()), this.memoryCacheKey);
        try {
    ...
    private Bitmap tryLoadBitmap() throws TaskCancelledException {
        ...
        try {
            ...
            if (imageFile != null && imageFile.exists() && imageFile.length() > 0) {
                C0126L.m33d(LOG_LOAD_IMAGE_FROM_DISK_CACHE, this.memoryCacheKey);
                this.loadedFrom = LoadedFrom.DISC_CACHE;
                checkTaskNotActual();
                bitmap = decodeImage(Scheme.FILE.wrap(imageFile.getAbsolutePath()));
            }
            if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
                C0126L.m33d(LOG_LOAD_IMAGE_FROM_NETWORK, this.memoryCacheKey);
                this.loadedFrom = LoadedFrom.NETWORK;
    ...
    private boolean tryCacheImageOnDisk() throws TaskCancelledException {
        C0126L.m33d(LOG_CACHE_IMAGE_ON_DISK, this.memoryCacheKey);
        try {
            ...
            C0126L.m33d(LOG_RESIZE_CACHED_IMAGE_FILE, this.memoryCacheKey);
    ...
    private boolean downloadImage() throws IOException {
        ...
        if (is == null) {
            C0126L.m34e(ERROR_NO_IMAGE_STREAM, this.memoryCacheKey);
        } else {
            try {
    ...
    private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
        ...
        if (!(bmp == null || this.configuration.processorForDiskCache == null)) {
            C0126L.m33d(LOG_PROCESS_IMAGE_BEFORE_CACHE_ON_DISK, this.memoryCacheKey);
            bmp = this.configuration.processorForDiskCache.process(bmp);
            if (bmp == null) {
                C0126L.m34e(ERROR_PROCESSOR_FOR_DISK_CACHE_NULL, this.memoryCacheKey);
            }
        }
        if (bmp == null) {
            return false;
    ...
    private boolean fireProgressEvent(int current, int total) {
        ...
        if (this.progressListener != null) {
            runTask(new C01231(current, total), false, this.handler, this.engine);
        }
        return true;
    }
    ...
    private void fireFailEvent(FailType failType, Throwable failCause) {
        if (!this.syncLoading && !isTaskInterrupted() && !isTaskNotActual()) {
            runTask(new C01242(failType, failCause), false, this.handler, this.engine);
        }
    }

    private void fireCancelEvent() {
        if (!this.syncLoading && !isTaskInterrupted()) {
            runTask(new C01253(), false, this.handler, this.engine);
        }
    }
    ...
    private boolean isViewCollected() {
        ...
        C0126L.m33d(LOG_TASK_CANCELLED_IMAGEAWARE_COLLECTED, this.memoryCacheKey);
        return true;
    }
    ...
    private boolean isViewReused() {
        ...
        C0126L.m33d(LOG_TASK_CANCELLED_IMAGEAWARE_REUSED, this.memoryCacheKey);
        return true;
    }
    ...
    private boolean isTaskInterrupted() {
        ...
        C0126L.m33d(LOG_TASK_INTERRUPTED, this.memoryCacheKey);
        return true;
    }
